...
tasks: `std::async`, `std::thread`, and `std::invoke` (the latter exposing 
...
struct two_way_executor
{
    ...
    void bulk_twoway_execute(F && f, std::size_t n, Ts &&... ts)
    {
...
{
    template <typename F, typename ...Ts>
    auto bulk_twoway_execute(F && f, std::size_t n, Ts &&... ts)
    {
        ...
        return exec.bulk_twoway_execute(std::forward<F>(f), n,
...
auto f = [](std::size_t, auto& shared_results, auto& shared_data) { ... };
auto result_factory = [](std::size_t n) {...};
auto shared_factory = []() {...};
...
future<std::result_of_t<F(Args...)>> foo(F && f, Args && ... args)
{
...
{
    return my_exec.twoway_execute(
        deferred_call(std::forward<F>(f), std::forward<Args>(args)...)
    );
}
```
...
future<std::result_of_t<F(Args...)>> foo(F && f, Args && ... args)
{
    return my_exec.twoway_execute(std::forward<F>(f), std::forward<Args>(args)...);
}
```
...
...
std::async(low, []{ std::cout << "1\n"; });
std::async(high, []{ std::cout << "9\n"; });
```

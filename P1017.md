...
The proposed executor interfaces are to be the fundamental bases for implementing
parallelism for todays and - more importantly - any future application. Thus
it is of utmost importance to come up with a design that is as flexible as 
possible and does not impose any limitations on possible future use use cases.

We pose that there is no strongly restricting reason for the executor interfaces
_not_ to be variadic. In our oppinion, all argumentation we have seen from the 
...
executors inhibits for those to express general use cases, especially for those 
...
executor may even have to perform that task to begin with.
...
Making the executor interfaces nullary requires to use a lambda which is error
...
From all of this we pose that making the interfaces variadic adds a small
